...
def get_invoices_by_customer_sorted_by_date(customer_id: int) -> str:
    ...
    query = text("SELECT invoice_id, invoice_date, total FROM chinook.invoice WHERE customer_id = :cid ORDER BY invoice_date DESC LIMIT 100")
...
def get_invoices_sorted_by_unit_price(customer_id: int) -> str:
    ...
    query = text("""
        ...
        JOIN chinook.invoice_line il ON i.invoice_id = il.invoice_id
        JOIN chinook.track t ON il.track_id = t.track_id
...
def get_employee_by_invoice_and_customer(invoice_id: int, customer_id: int) -> str:
    ...
    query = text("""
        ...
        JOIN chinook.customer c ON e.employee_id = c.support_rep_id
        JOIN chinook.invoice i ON c.customer_id = i.customer_id
